...
export interface SecretsActions {
  load: () => Promise<void>
  setValue: (key: keyof Pick<SecretsState, 'discordBotToken' | 'discordAppToken' | 'telegramBotToken' | 'slackBotToken' | 'slackAppToken'>, value: string) => void
  save: () => Promise<void>
...
export const useSecretsStore = create<SecretsStore>((set, get) => ({
  ...
  setValue: (key, value) => {
    set({ [key]: value, saveSuccess: false } as Partial<SecretsState>)
  },
