  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">{session.label ?? session.key}</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        {usage.modelUsage && usage.modelUsage.length > 0 && (
          <div className="mt-4">
            <p className="mb-2 text-xs font-medium text-muted-foreground">
              {t("usage.sessionDetail.modelsUsed")}
            </p>
              ...
              {usage.modelUsage.map((m, i) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-muted-foreground">
                    {t("usage.sessionDetail.modelCallsCost", {
                      n: m.count,
                      cost: m.totals.totalCost.toFixed(4),
                    })}
                  </span>
                </div>
              ))}
        ...
        {usage.toolUsage && usage.toolUsage.tools.length > 0 && (
          <div className="mt-4">
            <p className="mb-2 text-xs font-medium text-muted-foreground">
              {t("usage.sessionDetail.topTools")}
            </p>
