...
function toMetadata(now: number, obj: Record<string, unknown>): ClawUISessionMetadata | null {
  ...
  const tags = Array.isArray(obj.tags) ? obj.tags.filter((t) => typeof t === 'string').map((t) => t.trim()).filter(Boolean) : []
...
async function runOpenClawJson(args: string[], env: NodeJS.ProcessEnv, timeoutMs: number): Promise<unknown> {
...
export function registerMetadataHandlers(
  ...
  ipcMain.handle('metadata:generate', async (_event, sessionKeyInput: unknown): Promise<ClawUISessionMetadata> => {
    ...
    const firstText =
      Array.isArray(payloads) && payloads[0] && typeof (payloads[0] as { text?: unknown }).text === 'string'
    ...
    mainLog.info('[metadata.generate]', `sessionKey=${sessionKey}`, `durationMs=${Date.now() - t0}`)
    return finalMeta
  })
}
