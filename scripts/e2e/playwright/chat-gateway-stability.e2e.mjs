...
async function run() {
  ...
  const consoleZeroStep = Array.isArray(report.steps)
    ? report.steps.find((step) => step?.name === "renderer_console_error_zero")
    : null;
  if (!consoleZeroStep?.ok) {
    throw new Error(`renderer_console_error_zero is not ok. report=${reportPath}`);
  }
  if (
  ...
  if (Number(report?.checks?.rendererConsoleErrorCount ?? 0) > 0) {
    throw new Error(`renderer console has errors. report=${reportPath}`);
  }
  ...
  console.log(
    `[PASS] chat gateway stability e2e passed. screenshot=${screenshotPath} report=${reportPath}`,
  );
}
