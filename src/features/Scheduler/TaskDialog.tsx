...
interface TaskDialogProps {
  ...
  onSave: (task: Omit<ScheduledTask, "id" | "lastRun" | "nextRun" | "runCount">) => Promise<void>;
...
function cronToHumanReadable(
  ...
  if (minute !== "*" && hour === "*") {
    return t
      ? t("scheduler.taskDialog.cronReadable.everyHourAtMinute", { minute })
  ...
  if (minute !== "*" && hour !== "*" && dayOfMonth === "*" && dayOfWeek === "*") {
    ...
    return t
      ? t("scheduler.taskDialog.cronReadable.everyDayAtTime", { time })
      : `Every day at ${time}`;
  }
  ...
  if (minute !== "*" && hour !== "*" && dayOfMonth === "*" && dayOfWeek !== "*") {
    ...
    const time = `${hour.padStart(2, "0")}:${minute.padStart(2, "0")}`;
    return t
      ? t("scheduler.taskDialog.cronReadable.everyWeekdayAtTime", { day, time })
      : `Every ${day} at ${time}`;
  }
  ...
  if (minute !== "*" && hour !== "*" && dayOfMonth !== "*") {
    const time = `${hour.padStart(2, "0")}:${minute.padStart(2, "0")}`;
    return t
      ? t("scheduler.taskDialog.cronReadable.everyMonthDayAtTime", { dayOfMonth, time })
      : `Day ${dayOfMonth} of each month at ${time}`;
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent onClose={() => onOpenChange(false)}>
        <DialogHeader>
          <DialogTitle>
            {isEditing
              ? t("scheduler.taskDialog.titleEdit")
              : t("scheduler.taskDialog.titleCreate")}
          </DialogTitle>
          <DialogDescription>
